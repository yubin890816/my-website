...
Hero 区块 SHALL 包含一个名为"查看我的项目"的 CTA 按钮，点击后平滑滚动至页面内的 `#projects` 锚点（当目标存在且页面有可滚动空间时）。

- **GIVEN** 页面存在 `id="projects"` 的元素作为锚点目标，且该目标位置不在当前视口内
- **WHEN** 访客点击"查看我的项目"按钮
...
- **GIVEN** 页面 DOM 中存在 `<section id="projects">`，且该 section 有非零高度或页面 body 高度大于视口高度
- **WHEN** 访客点击 CTA 按钮
- **THEN** 视口滚动至该 section 顶部对齐视口顶部
- **AND** URL hash 更新为 `#projects`

#### Scenario: 锚点目标存在但无可滚动空间

- **GIVEN** 页面 DOM 中存在 `<section id="projects">`，但该 section 高度为 0 且页面 body 高度等于视口高度
- **WHEN** 访客点击 CTA 按钮
- **THEN** URL hash 更新为 `#projects`
- **AND** 页面不发生滚动（因物理上无可滚动空间）

#### Scenario: 锚点目标缺失时的降级

- **GIVEN** 因渲染异常导致 `#projects` 元素不存在于 DOM
- **WHEN** 访客点击 CTA 按钮
- **THEN** 浏览器 URL 不更新为无效锚点（CTA 的点击处理拦截默认行为）
